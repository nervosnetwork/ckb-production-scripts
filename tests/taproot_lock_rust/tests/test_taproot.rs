...
#[test]
fn test_key_path_spending_failed() {
    let mut data_loader = DummyDataLoader::new();

    let mut config = TestConfig::new();
    config.key_path_spending_failed = true;
...
#[test]
fn test_script_path_spending_y_parity_failed() {
    let mut data_loader = DummyDataLoader::new();

    let mut config = TestConfig::new();
    config.set_script_path_spending();
    config.y_parity_failed = true;
...
#[test]
fn test_script_path_spending_proof_failed() {
    let mut data_loader = DummyDataLoader::new();

    let mut config = TestConfig::new();
    config.set_script_path_spending();
    config.proof_failed = true;
    ...
    verifier.set_debug_printer(debug_printer);
    let verify_result = verifier.verify(MAX_CYCLES);
    assert_script_error(verify_result.unwrap_err(), ERROR_INVALID_PROOF);
}
...
#[test]
fn test_script_path_spending_failed() {
    let mut data_loader = DummyDataLoader::new();

    let mut config = TestConfig::new();
    config.set_script_path_spending();
    config.taproot_script_failed = true;
    ...
    verifier.set_debug_printer(debug_printer);
    let verify_result = verifier.verify(MAX_CYCLES);
    // this error code is returned from exec_script_0.c
    assert_script_error(verify_result.unwrap_err(), 1);
}

#[test]
fn test_script_path_spending_example_success() {
    let mut data_loader = DummyDataLoader::new();

    let mut config = TestConfig::new();
    config.set_script_path_spending();
    config.use_example_script = true;
