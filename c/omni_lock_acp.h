...
void process_amount(int min_ckb, int min_udt, uint64_t *min_ckb_amount,
                    uint128_t *min_udt_amount) {
  int is_overflow = quick_pow10(min_ckb, min_ckb_amount);
  if (is_overflow) {
    *min_ckb_amount = MAX_UINT64;
  }

  is_overflow = uint128_quick_pow10(min_udt, min_udt_amount);
  if (is_overflow) {
